...
| P-8 | **Recusa visível** (borda/nuvem/classe não calibrada → "liberação automática bloqueada · o caso segue para fotointerpretação manual") | Falso-negativo visível = critério de morte: na dúvida, o caso segue para fotointerpretação manual, e o sistema diz por quê. |

**Anti-fabricação na tela:** números ilustrativos de mockup marcados `· exemplo`; antecipação marcada `· projeção (não medido)` e em condicional ("**sinalizaria**", nunca "sinalizou"); nada de resultado medido apresentado como fato. **Anti-circularidade:** a antecipação é "antes de a **base de referência** atualizar" (nunca "vs PRODES") — PRODES/DETER é a verdade independente de rotulagem, jamais a base que se corre para bater. **Voz do sistema (teste de usuário 28/06):** só termos reais do CAR/SICAR (fotointerpretação manual, liberação automática, analista responsável, parecer), **sem travessão**, e o humano como autoridade que assina — a máquina **encaminha** o caso, **nunca "rebaixa"**. O trap do build derruba a compilação se `—`, `rebaix`, `seu nome no ato` ou qualquer item de anti-circularidade voltar.
