...
>
> **ENTREGA — sessão 28/06 (noite): pitch finalizado + equipe.** O produto não muda; isto registra o estado de entrega. **(1) Equipe (real, fornecida):** Mário Guilberto Machado Souza (Tecnólogo em Gestão da TI; Pós em Arquitetura e Gestão de Infraestrutura em TI), João Marcos Cassol (Analista de Segurança da Informação), Jackson Meires Dantas Canuto (Mestre em Eng. de Software; grad. Sistemas de Informação — **narra o áudio do pitch**). Fotos em `public/assets/team/`. **Nunca inventar nomes/papéis** (correção de uma fabricação desta sessão). **(2) Pitch = 10 slides na estrutura oficial da Live 08** (Introdução · Problema-quem-sofre · Problema-números · Solução · Como funciona · Diferenciais · Impacto · Time · Próximos passos · Encerramento) — a formulação "5 beats" abaixo fica como racional histórico; o deck operacional são esses 10 slides. Slide de produto usa **satélite Sentinel-2 real (antes/depois, caso rv7)** igual ao `/painel`; slide **Time** com fotos+credenciais. **(3) Voz natural/humana** (base = `apresentacao-mentores.html#perguntas`): **zero travessão** no deck (purgado no build) e **jargão interno fora da copy pública** — "Council", "roteador de atenção", "hierarquia de informação" não aparecem em pitch/site (vivem só aqui). **(4) Metas do backtest no deck = recall ≥0,90 / kappa ≥0,70** (calibrado contra PRODES/DETER, anti-circular: "antes da base ser atualizada"). As "≥2 meses / ≥80%" eram do bundle do designer, **não da proposta** — removidas. **Fonte canônica das claims do pitch = `apresentacao-mentores.html`.** **(5) Áudio:** Jackson lê `submission/roteiro-jackson.md` (corrido); campo de IA pronto em `submission/uso-de-ia.md`. O build `tools/build-pitch.cjs` **trava se não forem 10 slides ou se sobrar travessão**.
>
...
- **Copy do sistema (UI — regras duras, vindas de teste de usuário 28/06):** (1) **só termos que o CAR/SICAR já usam** (fotointerpretação manual, liberação automática, análise dinamizada, analista responsável, parecer, cena, talhão, delta, limiar) — **não inventar linguagem**; (2) **o humano é a autoridade que assina**; a máquina **encaminha** o caso a ela — **nunca enquadrar como rebaixamento** (`rebaixar`/"rebaixado para…": parece que a pessoa é "menos"). Na UI, usar o termo específico **`fotointerpretação manual`**; o conceito de estratégia **"revisão humana embutida"** segue válido nos docs — proibido é só o *verbo de rebaixamento*; (3) **sem travessão** ("—" lê como IA — usar ponto ou vírgula); (4) **registro profissional**, sem coloquialismo ("nada ainda", "no ato") nem frase redundante ("É o seu nome no ato"). O build do `/painel` (`tools/build-painel.cjs`) **derruba a compilação** se travessão, `rebaix` ou `seu nome no ato` reaparecerem.
- **O guia precisa de empatia E autoridade, sempre juntas.**
  ...
  - **Autoridade · "eu sei o que mudou":** fontes oficiais (PRODES/DETER + imagem datada, rastreável); sabe a regra (classes do Código Florestal); admite o limite (confiança baixa, o caso segue para fotointerpretação manual; autoridade sem blefe).
