...
- `painel.html` / `public/painel.html` + `support.js` / `public/support.js` — **Painel do Analista, protótipo interativo** (`/painel`, app React/DCLogic). Compilado por `tools/build-painel.cjs` do bundle `Gabarito - Painel do Analista (protótipo).dc.html`, com o mesmo **saneamento idempotente + trap + asserts**. Entrega: fila por risco (score invisível) com grupo **Concluídos**, **"o que mudou neste talhão"** por caso, **mapa gerado por talhão** (PRNG semeado pelo id) com **zoom/pan** e **antes/depois dramático**, **trilha de auditoria assinável**, rodapé de decisão fixo + responsivo `≤880px`. É a tela que o vídeo de protótipo filma ao vivo. Não editar à mão.
