...
"  _dims(){var el=document.querySelector('[data-tour=\"map\"]');if(!el)return false;var r=el.getBoundingClientRect();if(!r.width||!r.height)return false;this._mw=r.width;this._mh=r.height;this._ml=r.left;this._mt=r.top;return true;}\n" +
"  _clampPan(px,py,z){var W=this._mw,H=this._mh;return {x:Math.max(W-z*W,Math.min(0,px)),y:Math.max(H-z*H,Math.min(0,py))};}\n" +
"  _zoomBtn(d){if(!this._dims())return;var z=this.state.zoom,nz=Math.max(1,Math.min(5,d>0?z*1.4:z/1.4)),cx=this._mw/2,cy=this._mh/2,k=nz/z,px=cx-k*(cx-this.state.panX),py=cy-k*(cy-this.state.panY),p=this._clampPan(px,py,nz);if(nz===1){p.x=0;p.y=0;}this.setState({zoom:nz,panX:p.x,panY:p.y,_smooth:true});}\n" +
...
// regressão de zoom: nada de dimensão de mapa hardcoded; o zoom mede o elemento real
if (/_mw\s*\|\|\s*\d|_mh\s*\|\|\s*\d/.test(html)) throw new Error('ASSERT: dimensão de mapa hardcoded (||800/||600) voltou');
if (!html.includes("_dims(){var el=document.querySelector('[data-tour=\"map\"]')")) throw new Error('ASSERT: faltou _dims() medindo o elemento real');
