# Pacote de submissão — Gabarito · haCARthon 2026 · Desafio 02

Artefatos prontos para a submissão (e para a conversa de adoção). Produzidos em 28/06/2026.
Direção canônica: `../pm-role.md` (DIREÇÃO DECIDIDA). Deck: `../pitch.html` (`/pitch`).

## Mapa para os 3 entregáveis da Live 7

| Entregável (Live 7) | Formato exigido | Artefato neste pacote | Status |
|---|---|---|---|
| **Ideação** | Formulário appdesafios (nome, logo, resumo ≤300, próximos passos) | `ideacao-form.md` | ✅ texto pronto p/ colar |
| **Protótipo** | Vídeo **≤ 2 min**, YouTube público/não-listado, narração humana | `prototipo-script.md` (roteiro + shotlist) | ✅ roteiro pronto · falta gravar |
| **Pitch** | Slides **PDF estático + áudio**, ≤ 3 min | `../pitch.html` → `⌘/Ctrl+P` (PDF) · `pitch-narration.md` (áudio) | ✅ deck + narração prontos · falta gravar |

## Submissão dupla (artefato técnico + governança)

| Peça | Artefato | Para quê |
|---|---|---|
| Como a prova é feita | `backtest-recipe.md` | Método reproduzível do backtest de antecipação (dados reais, campos reais) |
| Como vira política | `minuta-nota-tecnica.md` | Instrumento que reparte a responsabilidade — torna a governança demonstrável, não promessa |
| Como começa | `backtest-recipe.md` §7 (e-mail SEMAD-GO) | O pedido datado de acesso ao t0 |

## O que ESTE pacote não resolve (honestidade — o multiplicador é seu)

1. **Uma frase de um(a) analista real de OEMA/SEMAD-GO.** Nenhum slide substitui "sim, assinar no escuro é o meu problema". É um e-mail (`backtest-recipe.md` §7) — só você pode disparar.
2. **Rodar o backtest com imagem real.** O método está pronto e é gratuito (CDSE free tier); falta executar.

## Mentoria (obrigatória — Live 7)
Registrar nome do mentor + feedback aqui antes de submeter: _________________
