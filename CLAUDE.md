...
**Verify a `/painel` change for real (a browser, not an assertion):** `chromium-browser --headless=new --no-sandbox --virtual-time-budget=15000 --dump-dom "<url-or-file://>" ` then grep the rendered DOM for `0` leftover `{{`, `0` `<x-dc`, the cases (`Rio Verde`), and your corrected strings. A boot test is the only proof the React runtime mounted; do not claim "it works" without it.
