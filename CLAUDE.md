...
`painel.html` / `public/painel.html` (`/painel`, "Painel do Analista") + `support.js`/`public/support.js` are **compiled by `tools/build-painel.cjs`** — same idempotent discipline as `build-pitch.cjs`. Sources (in `tools/`, gitignored-but-force-committed): `painel-bundle.dc.json` (raw DesignSync `get_file` of `Gabarito - Painel do Analista (protótipo).dc.html`) and `painel-support.js` (the unmodified DCLogic runtime — lazy-loads React+ReactDOM UMD from unpkg w/ SRI, auto-mounts `<x-dc>` on `DOMContentLoaded`). The script applies a **literal sanitize map + forbidden-string trap + positive asserts**, then writes all four files byte-identically; it is idempotent (double-run `md5sum` matches). Unlike `/pitch`, this is a genuinely stateful React/DCLogic app (queue, map epochs, guided tour, sign-off) so it can NOT be flattened to static HTML — keep the `<x-dc>` template + `<script type="text/x-dc" data-dc-script>` + `support.js` together.

**NEVER hand-edit `painel.html`** (the next `node tools/build-painel.cjs` or bundle re-pull silently reverts it — the exact whack-a-mole the build exists to kill). Corrections live in the script's sanitize map. Current sanitizations (auditable, with rationale in the script): **anti-circularity** — "antecipação vs PRODES" → "vs atualização da base · projeção" and the bar tag "PRODES anual" → "só na consolidação anual" (PRODES/DETER is the *independent truth for labeling*, never the baseline we race — `backtest-recipe.md` §4–6); **anti-fabrication** — illustrative queue count marked "· exemplo", antecipation marked "· projeção". The trap throws on `vs PRODES`/`PRODES anual`, fabricated-result strings (`Testamos`, …), and Alagoas placeholders.

**The map is generated per talhão** (ENRICH 2 in the build): a deterministic xorshift PRNG seeded by the case id produces a distinct parcel mosaic, land cover, river, pivots and focus parcel for each case — and the antes/depois toggle transforms the focused parcel **dramatically** (Base = intact nativa green + green wash; Sentinel-2 hoje = opaque exposed-soil scar + amber wash). Before this, the bundle drew the *same* static map for every case (owner: "troco de território e continuo no mesmo território"). Keep it generated; don't reintroduce static map layers. **Map is zoomable/pannable** (ENRICH 3): +/− buttons, scroll-wheel zoom toward cursor, drag-to-pan — a CSS transform on a wrapper around the land layers (state: `zoom/panX/panY`; mousemove via a window listener since the DCLogic template only binds onMouseDown/Up; zoom resets on case-select). **Decision buttons are a pinned `flex:none` footer** (ENRICH 1b) so they never clip; **`<=880px` responsive breakpoint** (ENRICH 4, `.ga-shell`/`.ga-main`) stacks the 3 columns and lets the page scroll on mobile. **"Concluídos" fila group** (ENRICH 5) lists decided cases with the decision + analyst (Luana A.).

**Verify a `/painel` change for real (a browser, not an assertion):** `chromium-browser --headless=new --no-sandbox --virtual-time-budget=15000 --dump-dom "<url-or-file://>"` then grep the rendered DOM for `0` leftover `{{`, `0` `<x-dc`, the cases (`Rio Verde`), parcel fills (`#2f4327`/`#b9a36e`), and your corrected strings. For visuals, `--screenshot=/tmp/x.png --window-size=1340,850` and Read the PNG. The tutorial auto-opens on first load and dims the map — to screenshot the cockpit, temp-patch `tourActive:!done`→`tourActive:false` (and `selectedId:'rv7'`→another id to see a different territory). A boot test is the only proof the React runtime mounted; do not claim "it works" without it.
