...
**The map is generated per talhão** (ENRICH 2 in the build): a deterministic xorshift PRNG seeded by the case id produces a distinct parcel mosaic, land cover, river, pivots and focus parcel for each case — and the antes/depois toggle transforms the focused parcel (Base = intact nativa green; Sentinel-2 hoje = terracotta scar). Before this, the bundle drew the *same* static map for every case (owner: "troco de território e continuo no mesmo território"). Keep it generated; don't reintroduce static map layers.
