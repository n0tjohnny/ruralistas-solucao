...
### How `/painel` is produced (interactive prototype — NOT statically compiled)

`painel.html` + `public/painel.html` (`/painel`, "Painel do Analista") is the Claude Design bundle `Gabarito - Painel do Analista (protótipo).dc.html` shipped **verbatim** plus its DCLogic runtime `support.js`/`public/support.js` (self-hosted; it lazy-loads React+ReactDOM UMD from unpkg with SRI and auto-mounts the `<x-dc>` template on `DOMContentLoaded`). Unlike `/pitch`, this one is genuinely stateful (queue, map epochs, tour, decisions) so it canNOT be flattened to static HTML — keep the `<x-dc>` template + `<script type="text/x-dc" data-dc-script>` component + `support.js` together. To update: re-pull the bundle via DesignSync, audit it against the anti-fabrication guardrail (the bundle data is already Goiás municípios + `CAR-GO-*` and carries the "dados ilustrativos (Goiás)" disclaimer), write both root+public copies, and redeploy. `support.js` is the unmodified design runtime — don't hand-edit it.
